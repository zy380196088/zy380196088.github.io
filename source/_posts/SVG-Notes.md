...
用 getSVGDocument()获取所引用的SVG的document对象.

使用document.createElement("line")创建一个SVG元素并添加到SVG文档中,该元素并不会显示出来.
因为创建SVG元素需要制定命名空间.需要在 svg 标签上设定 xmlns为 "http://www.w3.org/2000/svg";
正确的构造方式是调用 document.createElementNS("http://www.w3.org/2000/svg","line");
对SVG元素设定属性需要调用 setAttribute();

每个SVG元素对象上,都有一个getScreenCTM()方法,它会返回一个SVGMatrix来表示元素的坐标系所做过的变化.
SVG还有一种SVGPoint类型,它有x 和 y两个属性可以表示任意一个点,它还有一个 *** matrixTransform() ***方法可以将点跟某个SVGMartrix相乘得到相应矩阵变换后的的点.